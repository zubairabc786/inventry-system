...
export default function PurchaseBillManager() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const result = await getInventMasterSaleData(); // Direct assignment
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      setError("Failed to fetch Purchase or Sale data");
      console.error(err);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleUpdate = () => {
    ...
    router.push(`/sale-edit/${purchaseBillData.doc_id}`);
  };
  ...
  const generatePDF = (shouldPrint = false) => {
    ...
    // Subheading
    doc.setFontSize(16);
    doc.text("SALE BILL", 105, 28, { align: "center" });
    ...
    if (shouldPrint) {
      ...
      // Download the PDF
      doc.save(`Naveed-Fabrics-Sale-Bill-${purchaseBillData.doc_id}.pdf`);
    }
  };
