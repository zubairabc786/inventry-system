...
import {
  getSaleReturnBillData,
  getInventMasterSaleReturnId,
  getJournalEntries1,
  deletePurchaseSheet,
} from "../../action/action";
...
export default function PurchaseBillManager() {
  ...
  const [inventMasterSaleReturnData, setInventMasterSaleReturnData] = useState(
    []
  );
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const result = await getInventMasterSaleReturnId(); // Direct assignment
        setInventMasterSaleReturnData(result);
      } catch (err) {
  ...
  const handleSearch = async () => {
    ...
    try {
      const billData = await getSaleReturnBillData(searchDocId);
      ...
      setError("Failed to fetch sale return data");
      console.error(err);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleUpdate = () => {
    ...
    router.push(`/sale-return-edit/${purchaseBillData.doc_id}`);
  };
  ...
  const handleDelete = async () => {
    ...
    if (
      !confirm(
        "Are you sure you want to delete this sale return sheet? This action cannot be undone."
      )
    ) {
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        alert("Sale return sheet deleted successfully!");
        setShowBill(false);
        setPurchaseBillData(null);
        setSearchDocId("");
      } else {
        alert("Failed to delete sale return sheet: " + result.error);
      }
    } catch (error) {
      console.error("Delete error:", error);
      alert("Something went wrong while deleting the sale return sheet.");
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  const generatePDF = (shouldPrint = false) => {
    ...
    // Subheading
    doc.setFontSize(16);
    doc.text("Sale Return BILL", 105, 28, { align: "center" });
      ...
      <div className="bg-white rounded-xl shadow-md p-6 mb-6">
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <div className="flex-grow">
            ...
            <SaleReturnList
              coaList={inventMasterSaleReturnData}
