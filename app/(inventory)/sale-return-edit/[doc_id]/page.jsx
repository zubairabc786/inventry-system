            ...
            >
              {isSubmitting ? (
                ...
                <>
                  ...
                  <span>Update Sales Returns Invoice</span>
                </>
              )}
            </button>
          </div>
...
// export default function UpdatePurchaseSheet({ params }) {
//   const { doc_id } = params;
...
//       try {
//         const [{ coaList, productList }, purchaseSheetData] = await Promise.all(
//           [getDropdownData(), getPurchaseSheetById(doc_id)]
//         );

//         setCoaList(coaList);
//         setProductList(productList);

//         if (purchaseSheetData.success) {
//           const data = purchaseSheetData.data;
//           setDated(new Date(data.dated).toISOString().split("T")[0]);
//           setPurchaseCode(data.purchase_code);
//           setBillAmount(data.bill_amount);
//           setSaleMan(data.sale_man || "");
//           setCash(data.cash || "");
//           setJazzCash(data.jazz_cash || "");
...
//                 discount: {
//                   type: "percentage",
//                   value: item.calculated_discount || 0,
//                 },
//               }))
//             );

//             setDiscountValue(data.Purchase[0]?.calculated_discount || "");
//           }
//         }
//       } catch (error) {
...
//     formData.append("cash", cash);
//     formData.append("jazz_cash", jazz_cash);
//     formData.append("calculated_discount", discountValue);
//     formData.append("items", JSON.stringify(items));

//     try {
//       const result = await updatePurchaseSheet(doc_id, formData);

//       if (result.success) {
//         alert("Sale sheet updated successfully!");
//         router.push("/sale-return-detail");
//       } else {
//         alert("Failed to update sale sheet: " + result.error);
//       }
//     } catch (err) {
...
//             Sale Return Update Sheet
//           </h1>
//           <div className="absolute -bottom-2 left-0 right-0 h-1 bg-gradient-to-r from-yellow-400 to-orange-500 rounded-full transform scale-x-75"></div>
//         </div>
